...
void dump_index_specific_stats(ds2i::uniform_index const& coll,
	...
	ds2i::stats_line()("type", type)("log_partition_size",
...
void create_collection(InputCollection const& input,
		ds2i::global_parameters const& params, const char* output_filename,
...
//	size_t i = 0;
//	std::ofstream outd("./build/temp/bug/bug.docs");
//	std::ofstream outf("./build/temp/bug/bug.freqs");
//	uint32_t n = 1;
//	outd.write(reinterpret_cast<const char*>(&n), sizeof(int));
//	n = input.num_docs();
//	outd.write(reinterpret_cast<const char*>(&n), sizeof(int));
//	outd.flush();
...
//		i++;
//		if ((i > 5821110 && i < 5821122) || (i > 11925640 && i < 11925650)) {
//			std::cout << i << std::endl;
//			if (plist.docs.size() == 917826 || plist.docs.size() == 67662) {
//				n = plist.docs.size();
//				outd.write(reinterpret_cast<const char*>(&n), sizeof(int));
//				outf.write(reinterpret_cast<const char*>(&n), sizeof(int));
//				uint32_t const * eled = plist.docs.begin();
//				uint32_t const * elef = plist.freqs.begin();
//				while (eled < plist.docs.end()) {
//					outd.write(reinterpret_cast<const char*>(eled++),
//							sizeof(uint32_t));
//					outf.write(reinterpret_cast<const char*>(elef++),
//							sizeof(uint32_t));
//				}
//				outd.flush();
//				outf.flush();
