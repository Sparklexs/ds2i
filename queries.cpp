...
void op_perftest(IndexType const& index,
	...
	for (size_t run = 0; run <= runs; ++run) {
		...
		if (run == runs) {
			std::string filename = "/home/xsong/Dataset/" + index_type;
			filename += ".csv";
			std::ofstream outcome(filename, std::ofstream::app);
			for (auto t : query_times) {
				outcome << index_type << "," << query_type << "," << t << "\n";
			}
			std::sort(query_times.begin(), query_times.end());
			double avg = std::accumulate(query_times.begin(), query_times.end(),
					double()) / query_times.size();
...
//		logger() << "95% quantile: " << q95 << std::endl;
//
//		stats_line()("type", index_type)("query", query_type)("avg", avg)("q50",
//				q50)("q90", q90)("q95", q95);
//	}
}
...
void perftest(const char* index_filename, const char* wand_data_filename,
		std::vector<ds2i::term_id_vec> const& queries, std::string const& type,
...
int main(int argc, const char** argv) {
	...
	term_id_vec q;
	/* if debug, we can load queries from files rather than std::in */
	//////////////////////////////////
	//	ifstream quer(argv[5]);
	//	std::string line;

	//	while (std::getline(quer, line)) {
