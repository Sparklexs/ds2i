...
//			uint32_t* docFreq_endpoints = (uint32_t*) m_block_endpoints;
			for (size_t b = 0; b < m_blocks; ++b) {
//				static uint32_t docFreq_length_pre = 0;
				blocks.emplace_back();
				uint32_t cur_block_size =
						((b + 1) * block_size <= size()) ?
								block_size : (size() % block_size);
				...
				ptr = BlockCodec::decode(freq_ptr, buf.data(), uint32_t(-1),
						cur_block_size);
				blocks.back().end = ptr;
//				blocks.back().end = ptr = blocks.back().docs_begin
//						+ *docFreq_endpoints - docFreq_length_pre;
//				docFreq_length_pre = *docFreq_endpoints++;
			}
