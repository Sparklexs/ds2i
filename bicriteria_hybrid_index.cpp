...
template<typename InputCollectionType>
struct block_merger: ds2i::semiasync_queue::job {
	block_merger(block_id_type block_id_base,
			typename InputCollectionType::document_enumerator e,
			ds2i::predictors_vec_type const& predictors,
			std::vector<uint32_t> const& counts, ds2i::progress_logger& plog,
			block_lambdas_type & block_doc_freq_lambdas,
			std::vector<int>::iterator index_it, double _mu, bound_type _type) :
			m_block_id_base(block_id_base), m_e(e), m_predictors(predictors), m_counts(
					counts), m_plog(plog), m_lambdas(block_doc_freq_lambdas), m_it(
					index_it), mu(_mu), m_bound_type(_type) {

	}
	virtual void prepare() {
		// blocks are organized by staggered docid and
		// freq. We have to re-compose this structure
		// to merge these blocks while keep docid and
		// freq staggered.
		using namespace ds2i;
		using namespace time_prediction;

		auto num_blocks = m_e.num_blocks();
		auto num_tuples = m_e.num_blocks() / 2;
		int tuples_left = num_tuples / 2 - 1; // isolate the first block

		// calculate the minimal cost of single blocks.
		std::vector<double> min_cost(num_tuples);
		for (int i = 0; i < num_blocks;) {
			if (m_bound_type == TIME) {
				// docid-block
				min_cost[i] = m_lambdas[m_block_id_base + i][m_it[i]].st.space
						+ mu * m_lambdas[m_block_id_base + i][m_it[i]].st.time;
				// freq-block
				min_cost[i] +=
						m_lambdas[m_block_id_base + i + 1][m_it[i + 1]].st.space
								+ mu
										* m_lambdas[m_block_id_base + i + 1][m_it[i
												+ 1]].st.time;
			} else {
				// docid-block
				min_cost[i] = mu
						* m_lambdas[m_block_id_base + i][m_it[i]].st.space
						+ m_lambdas[m_block_id_base + i][m_it[i]].st.time;
				// freq-block
				min_cost[i] +=
						mu
								* m_lambdas[m_block_id_base + i + 1][m_it[i + 1]].st.space
								+ m_lambdas[m_block_id_base + i + 1][m_it[i + 1]].st.time;
			}
			i += 2;
		}
		...
		// compare single with merged, skip the first two blocks
		for (int i = 0; i < tuples_left;) {
			// outer variables
			double currentPhi = 0;
			block_id_type base = m_block_id_base + i;

			ds2i::time_prediction::feature_vector fv_doc, fv_freq;
			double time = 0;

			fv_doc[feature_type::size] = m_lambdas[base][m_it[i]].st.space;
			fv_freq[feature_type::size] =
					m_lambdas[base + 1][m_it[i + 1]].st.space;

			fv_doc[feature_type::sum_of_logs] =
					m_lambdas[base][m_it[i]].st.sum_of_logs;
			fv_freq[feature_type::sum_of_logs] =
					m_lambdas[base + 1][m_it[i + 1]].st.sum_of_logs;

			fv_doc[feature_type::nonzeros] =
					m_lambdas[base][m_it[i]].st.nonzeros;
			fv_freq[feature_type::nonzeros] =
					m_lambdas[base + 1][m_it[i + 1]].st.nonzeros;

			fv_doc[feature_type::max_b] = m_lambdas[base][m_it[i]].st.max_b;
			fv_freq[feature_type::max_b] =
					m_lambdas[base + 1][m_it[i + 1]].st.max_b;

			for (int j = 1;
					j <= tuples_left && j < ds2i::mixed_block::BLOCKSIZE_K;
					j++) {
				// predict the space and time of merged blocks
				fv_doc[feature_type::size] += m_lambdas[base + 2 * j][m_it[i
						+ 2 * j]].st.space;
				fv_doc[feature_type::sum_of_logs] +=
						m_lambdas[base + 2 * j][m_it[i + 2 * j]].st.sum_of_logs;
				fv_doc[feature_type::nonzeros] += m_lambdas[base + 2 * j][m_it[i
						+ 2 * j]].st.nonzeros;
				fv_doc[feature_type::max_b] += m_lambdas[base + 2 * j][m_it[i
						+ 2 * j]].st.max_b;

				fv_freq[feature_type::size] +=
						m_lambdas[base + 2 * j + 1][m_it[i + 2 * j + 1]].st.space;
				fv_freq[feature_type::sum_of_logs] +=
						m_lambdas[base + 2 * j + 1][m_it[i + 2 * j + 1]].st.sum_of_logs;
				fv_freq[feature_type::nonzeros] +=
						m_lambdas[base + 2 * j + 1][m_it[i + 2 * j + 1]].st.nonzeros;
				fv_freq[feature_type::max_b] +=
						m_lambdas[base + 2 * j + 1][m_it[i + 2 * j + 1]].st.max_b;

				if (m_bound_type == TIME) {
					currentPhi =
							m_lambdas[base + j - 1][m_it[i + j - 1]].st.space
									+ mu
											* m_lambdas[base + j - 1][m_it[i + j
													- 1]].st.time;
				}
			}
		}
	...
	block_lambdas_type & m_lambdas;
	std::vector<int>::iterator m_it;
	double mu;
	bound_type m_bound_type;
